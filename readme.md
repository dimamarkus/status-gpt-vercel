...
The [Stripe CLI](https://stripe.com/docs/stripe-cli#install) `fixtures` command executes a series of API requests defined in a JSON file. To speed up the setup, we have added a [fixtures file](lib/fixtures/stripe-fixtures.json) to bootstrap test product and pricing data in your Stripe account. Simply run `stripe fixtures lib/fixtures/stripe-fixtures.json`.
...
```bash
supabase gen types typescript --project-id PROJECT_REFERENCE_ID --schema public > types_db.ts
```
