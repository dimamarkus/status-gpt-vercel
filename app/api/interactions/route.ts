...
import { APIApplicationCommandInteraction, APIEmbed, APIInteraction, APIInteractionResponse } from "discord-api-types/v10"
import withDiscordInteraction from "#/lib/middlewares/discord-interaction"
...
const getEmbed = async (value: string) => {
  switch (value) {
    case "cat":
      ...
      const { url } = await catResponse.json();
...
const handler = async (
  ...
  interaction: any // TODO - should be APIApplicationCommandInteraction
) => {
  const {
    data: { name, options },
  } = interaction
